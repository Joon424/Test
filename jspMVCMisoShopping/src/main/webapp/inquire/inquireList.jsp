...
function answerOpen(inquireNum) {
	if($("#inq_"+inquireNum).text()=='답변 보기') {
		$("#inq_"+inquireNum).text('답변 닫기');
		$("#inq"+inquireNum).css("display", "");
	}else{
		$("#inq_"+inquireNum).text('답변 보기');
...
<c:forEach items="${list }" var="dto">		
	...
	<tr><td colspan="3">${fn:replace( dto.inquireContent, newLine, "<br />") }
		<c:if test="${! empty dto.inquireAnswer }">
		<button type = "button" style="float:right;" id = "inq_${dto.inquireNum}" onclick="answerOpen('${dto.inquireNum}')">답변 보기</button>
		</c:if>
		</td></tr>
	<c:if test = "${! empty dto.inquireAnswer }">
